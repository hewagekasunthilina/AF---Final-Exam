...
class EditItemsPage extends React.Component {
    ...
    updateItem(item) {
        axios.put('/items', {item})
            ...
            .catch(error => {
                alert(`Unable to update ${item.title} because ${error}`)
            });
    }
    ...
    deleteItem(item) {
        axios.delete(`/items/${item._id}`)
            ...
            .catch(error => {
                alert(`Unable to delete ${item.title} because ${error}`)
            });
    }
    ...
    fetchItems() {
        axios.get('/items')
            .then(res => {
                this.setState({items: res.data.body});
            })
    }
        ...
        return (
            <div className="row align-items-center">
                {this.state.items.map((item, index) => {
                    return <EditItem item={item} categories={this.state.categories} updateItem={this.updateItem}
